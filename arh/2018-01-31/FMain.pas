...
type
  TFormMain = class(TForm)
    ...
    procedure setWeightValue(b0: Byte; Massa: currency);
  ...
  private
    id1, id2: Integer;
    procedure sendData(Weight: currency);
...
var
  ...
  file_name: string;
begin
  file_name := ChangeFileExt(ExtractFilePath(Application.ExeName)
      + ExtractFileName(Application.ExeName), '') + '.ini';
  INIFile := TINIFile.Create(file_name);

//  Host := trim(INIFile.ReadString('server', 'host_url', ''));
//  if (Host = '') then
//  begin
//    while Host = '' do
//      Host := trim(InputBox('HOST URL', 'Input URL to the data host',
//          'https://host_name.com'));
//    INIFile.WriteString('server', 'host_url', Host);
//  end;
//  if (Host[length(Host)] <> '/') then
//    Host := Host + '/';
//  Host := Host + 'weigher/set_weight';

  Title := trim(INIFile.ReadString('application', 'title', ''));
  if (Title = '') then
  begin
    INIFile.WriteString('application', 'title', InputBox('APPLICATION TITLE',
        'Input application title', 'velox_app'));
    Title := trim(INIFile.ReadString('application', 'title', ''));
  end;
...
//    if (iB[0] = STATUS_SUCCESS) then begin
      if CurrentWindow = 0 then CurrentWindow := getWindowHWND;
...
procedure TFormMain.setWeightValue(b0: Byte; Massa: currency);
begin
  with FormWeigher do
  begin
    FormWeigher.BringToFront;
    LabelWeigher.Caption := FormatFloat('0.000', Massa);
    case b0 of
      // If "Zero"
      STATUS_ZIRO:
        begin
...
procedure TFormMain.sendData(Weight: currency);
var
  CurrentDate: string;
  result: boolean;
begin
  CurrentDate := DateToStr(now) + '   ' + TimeToStr(now);

  result := SetWindowText(CurrentWindow,
      TitleOrigin +
      ' #Weight=' + FormatFloat('0.000', Weight) +
      ' #Date=' + DateToStr(now) + '   ' + TimeToStr(now)
  );
...
begin
  ...
  while Wnd <> 0 do
  begin
    if (Wnd <> Application.handle) and IsWindowVisible(Wnd) // Невидимые окна
      and (GetWindow(Wnd, gw_owner) = 0) // Дочерние окна
      and (GetWindowText(Wnd, buff, SizeOf(buff)) <> 0) then
    begin
      GetWindowText(Wnd, buff, SizeOf(buff));
      tmpTitle := StrPas(buff);
      if pos(Title, UpperCase(tmpTitle)) <> 0 then
      begin
        TitleOrigin := tmpTitle;
        Result := Wnd;
        break;
      end;
    end;
...
begin
  ...
  RegisterHotKey(Handle, id1, MOD_CONTROL, VK_W);

  // Register Hotkey Ctrl + Alt + W
  id2 := GlobalAddAtom('Hotkey2');
  RegisterHotKey(Handle, id2, MOD_CONTROL + MOD_Alt, VK_W);
end;
...
//    ShowMessage('Ctrl + A was pressed !');
  if Msg.HotKey = id2 then
    ShowMessage('Ctrl + Alt + W was pressed !');
end;
...
begin
  h := GetActiveWindow;
  if GetActiveWindow = CurrentWindow then begin
    for i := 1 to Length(sWeight) do
      SendMessage(CurrentWindow, WM_CHAR, Word(sWeight[i]), 0);
  end;
end;
